...
### OOM Error ko kaise diagnose kare

Sabse pehle Pata lagao asli wajah kya hai.

Sabse pehle tumhe ye confirm karna hai ki error Container Limit ki wajah se hai ya Node Pressure ki wajah se.

**Step 1: Pod ka Status Check Karein**:
...
Agar status ```CrashLoopBackOff``` ya ```Error``` dikh raha hai, to next step par jao.

<br>

**Step 2: Pod ki History Check Karein**:

```
kubectl describe pod <pod_name>
```

Iske output mein Containers section ke andar Last State ko dhundo.
- Agar Reason: OOMKilled hai: Iska matlab hai ki container ne apni Limit cross ki hai.
- Agar Exit Code: 137 hai: Ye confirm karta hai ki system ne use RAM ki wajah se mara hai.

<br>

**Step 3: Node ka Health Check Karein**:

Agar pod bar-bar "Evict" ho raha hai, to node ko check karo:
```
kubectl get nodes
```

Agar status mein ```MemoryPressure``` dikh raha hai, to samajh jao ki node ki khud ki RAM khatam ho gayi hai.

<br>

**Step 4: Metrics Dekho (Real-time Usage)**:

Agar tumne Metrics Server install kiya hai, to ye command bohot kaam aati hai:
```
kubectl top pod <pod_name>  # Pod kitni RAM le raha hai
kubectl top node            # Node kitni RAM le raha hai
```

<br>
<br>

### OOM Error ko kaise Fix kare

Diagnosis ke baad solution do tarah ke ho sakte hain: Quick Fix (Temporary) aur Permanent Fix.

<br>

**1 - Agar Container Limit Cross ho rahi hai (Container OOM)**:

- **Quick Fix**: Pod ki YAML file mein ```resources.limits.memory``` ko badha do. Agar pehle ```512Mi``` tha, to use ```1Gi``` karke dekho.

- **Permanent Fix**: * Memory Leak Check: Apne code mein check karo ki kya koi objects ya variables RAM mein phase reh gaye hain? (Python mein ```tracemalloc``` ya Node.js mein ```heapdump``` use karo).
  - **VPA (Vertical Pod Autoscaler)**: Ise setup karo. Ye apne aap check karega ki tumhara pod kitni RAM consume kar raha hai aur limits ko auto-adjust karne ki recommendation dega.
 
<br>

**2 - Agar Node ki RAM khatam ho rahi hai (Node OOM)**:

- **Memory increase karo**: Cloud VM ko resize karo. Cloud Provider (Azure/AWS) ko bolo ki naya node add kar de.

- **Taints and Tolerations**: Heavy pods ko specific nodes par restrict kar do taaki wo dusre light-weight pods ko distrub na karein.
