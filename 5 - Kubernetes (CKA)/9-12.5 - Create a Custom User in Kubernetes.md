...
### 2 - Kubernetes Native (Certificate-based Local User)
...
### 3 - Service Account + kubeconfig

**Concept**:
- Normal human users ke liye recommended nahi, lekin possible hai.
- Tum ek service account banao jiski RBAC me sirf pods & deployments read-only permission ho.
- Uska token nikalke user ko de do, aur kubeconfig me set kar do.

**Flow**:
- ServiceAccount create karo (namespace scoped).
- Role + RoleBinding banao.
- Secret se token lo (```kubectl get secret ...```).
- User ka kubeconfig me token embed karo.

**Example Lab Setup**:

**Step 1 – Service Account Create Karna**:

Kubernetes cluster main admin ko ek service account create karna hoga. Service Account ek namespace specific hota hai, to hum “default” namespace me bana lete hain (tum chaaho to koi alag namespace bhi use kar sakte ho).
...
Example:
```
kubectl apply -f puneet-sa.yaml
```

**Step 2 – Role Banana (Pods aur Deployments Read Only)**:

Kubernetes cluster main admin ko ab ek role create karna hai. Role namespace-specific hota hai, to hum default namespace me ek role banate hain.
...
metadata:
  namespace: default
  name: puneet-read-role
rules:
- apiGroups: [""] # "" means core API group (pods)
  resources: ["pods"]
  verbs: ["get", "list", "watch"]
...
Apply:
```
kubectl apply -f puneet-role.yaml
```

**Step 3 – Role ko Service Account se Bind Karna**:

Kubernetes cluster main admin ko ab role binding create karni hai. RoleBinding ka use hota hai Role ko Service Account ya user ke saath link karne ke liye.

Example:
```
# puneet-rolebinding.yaml
apiVersion: rbac.authorization.k8s.io/v1
kind: RoleBinding
metadata:
  name: puneet-read-rolebinding
  namespace: default
subjects:
- kind: ServiceAccount
  name: puneet-sa
...
Apply:
```
kubectl apply -f puneet-rolebinding.yaml
```

**Step 4 – Service Account ka Token Lena**:

Kubernetes me service account token ek Secret ke form me store hota hai (in AKS me ye usually JWT token hota hai).

Pehle secret ka naam find karo:

Example:
```
kubectl get secret -n default | grep puneet-sa
```

Output:
```
puneet-sa-token-abcde   kubernetes.io/service-account-token   3      2m
```

Token extract karo:
```
kubectl get secret puneet-sa-token-abcde -n default -o jsonpath='{.data.token}' | base64 --decode
```
Ye token tumhare “password” ki tarah kaam karega.


**Step 5 – CA Certificate Lena**:

Cluster ka CA certificate kubeconfig me chahiye hota hai.

Example:
```
kubectl get secret puneet-sa-token-abcde -n default -o jsonpath='{.data.ca\.crt}' | base64 --decode > ca.crt
```

**Step 6 – Cluster Server Endpoint Lena**:

Tumhare AKS cluster ka API server URL ye command se milega:
...
Output:
```
https://myakscluster-dns-1234.hcp.eastus.azmk8s.io:443
```

**Step 7 – Puneet ke liye Custom kubeconfig File Banana**:

Ab hum ek alag kubeconfig banayenge jo Puneet ke access ke liye hai.

Example:
```
# puneet-kubeconfig.yaml
apiVersion: v1
kind: Config
clusters:
- name: aks-cluster
  cluster:
    ...
    server: https://myakscluster-dns-1234.hcp.eastus.azmk8s.io:443
contexts:
- name: puneet-context
  context:
    cluster: aks-cluster
    namespace: default
    user: puneet-user
current-context: puneet-context
users:
- name: puneet-user
  user:
...
**Note**:
- ```<BASE64_ENCODED_CA_CERT>``` me ```ca.crt``` ka base64 encode data hoga:
```
cat ca.crt | base64 | tr -d '\n'
```
- ```<SERVICE_ACCOUNT_TOKEN>``` me step 4 wala token paste karna hai.

**Step 8 – Puneet ko Kubeconfig Dena**:

Ab tum Puneet ko ye ```puneet-kubeconfig.yaml``` file doge.

Woh apne system me ye command chalaye:
Example:
```
export KUBECONFIG=/path/to/puneet-kubeconfig.yaml
kubectl get pods
kubectl get deployments
kubectl delete pod xyz   # Ye fail hoga (Access Denied)
```

**Flow Summary**:
- Tumne ek Service Account banaya (robot user).
- Tumne ek Role banaya jo sirf pods aur deployments read kare.
- Tumne RoleBinding banaya jo Role ko Service Account se link kare.
- Tumne Service Account ka token nikala.
- Tumne CA cert aur API endpoint nikale.
- Tumne ek custom kubeconfig banaya jisme token use hua.
- Ye kubeconfig Puneet ko diya.
- Puneet ab cluster me sirf read-only access le sakta hai pods aur deployments ka.
...
| Method                     | Auth Mechanism  | Best For           | Security | Effort |
| -------------------------- | --------------- | ------------------ | -------- | ------ |
| **Azure AD User**          | Azure AD token  | Corporate users    | High     | Medium |
| **Certificate-based User** | X.509 mTLS      | Non-AAD env / labs | Medium   | Medium |
| **Service Account**        | Static token    | Quick temp access  | Low      | Low    |
| **External IdP**           | OIDC/SAML token | Multi-cloud SSO    | High     | High   |
